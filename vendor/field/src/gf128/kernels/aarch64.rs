...
/// Batch multiply 2× Gf128 in parallel.
///
...
/// `aes` is statically enabled or has been runtime-detected.
#[target_feature(enable = "aes")]
pub unsafe fn ghash_mul_vec2_neon(a: [Gf128; 2], b: [Gf128; 2]) -> [Gf128; 2] {
...
/// `aes` is statically enabled or has been runtime-detected.
#[target_feature(enable = "aes")]
pub unsafe fn ghash_mul_unreduced_neon(a: Gf128, b: Gf128) -> Gf128Product {
    // SAFETY: function carries the aes target feature.
    unsafe {
        let p_ll = pmull(a.lo, b.lo);
        let p_lh = pmull(a.lo, b.hi);
        ...
        Gf128Product::from_polynomial_words([ll_lo, ll_hi ^ cr_lo, hh_lo ^ cr_hi, hh_hi])
    }
}
...
#[target_feature(enable = "aes")]
pub unsafe fn ghash_square(a: Gf128) -> Gf128 {
    // SAFETY: function carries the aes target feature.
    unsafe {
...
// Retained Flock genus95 round-one register accumulator.
...
#[derive(Clone, Copy)]
pub struct Gf128NeonAccumulator([uint64x2_t; 3]);
impl Gf128NeonAccumulator {
    #[inline(always)]
    pub fn zero() -> Self {
...
///
/// # Safety
/// Requires AES/PMULL (the module is compiled only with this target feature).
#[inline(always)]
pub unsafe fn mul_acc_unred(acc: &mut Gf128NeonAccumulator, eq: Gf128, x: uint64x2_t) {
    unsafe {
        let xl = vgetq_lane_u64::<0>(x);
        let xh = vgetq_lane_u64::<1>(x);
        acc.0[0] = veorq_u64(acc.0[0], pmull_u64(eq.lo, xl));
        acc.0[1] = veorq_u64(
            acc.0[1],
            veorq_u64(pmull_u64(eq.lo, xh), pmull_u64(eq.hi, xl)),
        );
        acc.0[2] = veorq_u64(acc.0[2], pmull_u64(eq.hi, xh));
    }
}

/// Fold an [`Gf128NeonAccumulator`] to `(r0..r3)` and reduce mod p (same math as
/// [`crate::Gf128Product::reduce`], so results are bit-identical).
#[inline]
pub fn reduce_unred(acc: &Gf128NeonAccumulator) -> Gf128 {
    unsafe {
        Gf128Product::from_polynomial_words([
            vgetq_lane_u64::<0>(acc.0[0]),
            vgetq_lane_u64::<1>(acc.0[0]) ^ vgetq_lane_u64::<0>(acc.0[1]),
            vgetq_lane_u64::<0>(acc.0[2]) ^ vgetq_lane_u64::<1>(acc.0[1]),
            vgetq_lane_u64::<1>(acc.0[2]),
        ])
        .reduce()
    }
}

/// Multiply by an explicit polynomial of degree at most 63 (the NTT twiddle kernel).
#[inline]
pub fn mul_polynomial_u64(v: Gf128, t_lo: u64) -> Gf128 {
    use core::arch::aarch64::*;
    unsafe {
