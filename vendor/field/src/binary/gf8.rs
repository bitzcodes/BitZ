...
#[inline]
pub(super) fn clmul8(a: u8, b: u8) -> u16 {
...
const fn clmul8_software(a: u8, b: u8) -> u16 {
    ...
    let mut i = 0;
    while i < 8 {
        acc ^= (b16 << i) & 0u16.wrapping_sub(((a >> i) & 1) as u16);
        i += 1;
    }
    acc
}
...
mod tests {
    ...
    #[test]
    fn add_is_xor() {
        assert_eq!(Gf8(0x53) + Gf8(0xCA), Gf8(0x53 ^ 0xCA));
        assert_eq!(Gf8(0xFF) + Gf8(0xFF), Gf8::ZERO);
    }
    ...
    #[test]
    fn mul_known_values() {
        // x = Gf8(0x02). x^2 = 0x04. x^4 = 0x10.
        // x^8 mod p = x^4 + x^3 + x + 1 = 0x1B.
        let x = Gf8(0x02);
        let x2 = x * x;
        let x4 = x2 * x2;
        let x8 = x4 * x4;
        assert_eq!(x2, Gf8(0x04));
        assert_eq!(x4, Gf8(0x10));
        assert_eq!(x8, Gf8(0x1B));
    }
    ...
    fn inv_roundtrip() {
        for v in 1u8..=255 {
            let a = Gf8(v);
            assert_eq!(a * a.inverse_or_zero(), Gf8::ONE, "v={}", v);
        }
    }
    ...
    fn associativity_random() {
        ...
        for _ in 0..256 {
            let a = Gf8((rng.next_u64() & 0xff) as u8);
            let b = Gf8((rng.next_u64() & 0xff) as u8);
            let c = Gf8((rng.next_u64() & 0xff) as u8);
    ...
    fn fips_197_test_vectors() {
        ...
        assert_eq!(Gf8(0x57) * Gf8(0x13), Gf8(0xfe), "FIPS-197: 57·13");
        assert_eq!(Gf8(0x57) * Gf8(0x83), Gf8(0xc1), "FIPS-197: 57·83");
    ...
    fn neon_gf8_mul_vec16_matches_scalar() {
        ...
        for _ in 0..256 {
            ...
            // Scalar reference: lane-wise Gf8 mul.
            let mut expected = [0u8; 16];
            for i in 0..16 {
                expected[i] = (Gf8(a_arr[i]) * Gf8(b_arr[i])).0;
            }
            // NEON result.
    ...
    fn fermat_little_theorem() {
        ...
        for v in 1u8..=255 {
            ...
            assert_eq!(p, Gf8::ONE, "a^255 != 1 for a=0x{v:02x}");
        }
    }
}
